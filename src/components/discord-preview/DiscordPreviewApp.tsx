...
export default function DiscordPreviewApp() {
  ...
  const [hoverPreviewData, setHoverPreviewData] = useState<DiscordPreviewData | null>(null);
  ...
  const handleSave = (name: string) => {
    ...
    setSavedMessages((prev) => {
      ...
      const filtered = prev.filter(m => m.name.toLowerCase() !== name.toLowerCase());
          ...
          <div className="custom-scrollbar max-h-[85vh] overflow-y-auto pr-2">
            ...
            {tab === "saved" && (
              <StorageSidebar
                ...
                onLoad={(msg) => setModalState({ isOpen: true, pendingMsg: msg })}
                  ...
                  <div className="mb-4 flex items-center justify-between px-1">
                    <div>
                      ...
                      <p className="mt-1 text-[10px] text-white/20">
                        Edita el estado de la aplicación directamente.
                      </p>
                    </div>
                    <button
                      onClick={handleCopy}
          ...
          <div className="sticky top-24">
            <div className="mb-4 flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                <div className={`h-2 w-2 rounded-full transition-all duration-500 ${lastAutoSave ? 'bg-emerald-500 shadow-sm shadow-emerald-500/50' : 'animate-pulse bg-orange-500/50'}`} />
                <span className="text-[11px] font-semibold tracking-widest text-white/40 uppercase">
                  {hoverPreviewData ? 'Vista previa (Guardado)' : 'Vista previa en vivo'}
                </span>
                {lastAutoSave && (
                   <span className="text-[9px] text-white/10 italic">
                     Auto-guardado {new Date(lastAutoSave).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                   </span>
                )}
              </div>
            ...
            <div className={`overflow-hidden rounded-[24px] border transition-all duration-300 ${hoverPreviewData ? 'border-orange-500/30 ring-1 ring-orange-500/20' : 'border-white/6'} bg-[#313338] shadow-2xl shadow-black/50`}>
